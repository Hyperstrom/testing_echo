...
public class StatusUpdateValidationTest {
	...
	// XPath for the profile image on the home page (used to verify successful login
	// in setup)
	private final By PROFILE_IMAGE_ON_HOMEPAGE = By
			.xpath("//*[@id=\"root\"]/div/div/div[2]/div[1]/div/div/a[1]/div/img");
	...
	// Assuming a span or div with an 'error' class for validation messages within
	// the post form
	private final By STATUS_VALIDATION_MESSAGE_XPATH = By.xpath(
			"//form[contains(@class, 'post-form')]//span[@class='error'] | //form[contains(@class, 'post-form')]//div[@class='error-message'] | //*[contains(text(),'cannot be empty') or contains(text(),'required')]");
	// post time verification
	private final By POST_TIME_XPATH = By
			.xpath("//*[@id=\"root\"]/div/div/div[2]/div[2]/div/div[3]/div[1]/div/div[1]/div/div/span");

	// NEW XPaths for commenting functionality
	private final By COMMENT_INPUT_BOX_XPATH = By
			.xpath("/html/body/div/div/div/div[2]/div[2]/div/div[3]/div[1]/div/div[4]/div[1]/input");
	private final By COMMENT_SEND_BUTTON_XPATH = By
			.xpath("/html/body/div/div/div/div[2]/div[2]/div/div[3]/div[1]/div/div[4]/div[1]/button");
	private final By COMMENT_TOGGLE_BUTTON_XPATH = By
			.xpath("/html/body/div/div/div/div[2]/div[2]/div/div[3]/div[1]/div/div[3]/div[2]");
	...
	public void setup() {
		...
		try {
			...
			Assert.fail("Setup failed: Could not log in or reach homepage within " + WAIT_TIMEOUT_SECONDS + " seconds. "
					+ e.getMessage());
		}
	}
	...
	public void testEmptyStatusUpdateValidation() {
		...
		try {
			...
			Assert.fail("Failed to find status update input field within " + WAIT_TIMEOUT_SECONDS + " seconds. "
					+ e.getMessage());
		}
		...
		try {
			...
			Assert.fail("Failed to find or click 'Share' button within " + WAIT_TIMEOUT_SECONDS + " seconds. "
					+ e.getMessage());
		}

		// 3. Verify validation message appears. If it does NOT appear, the test should
		// FAIL.
		...
		try {
			...
			WebElement validationMessage = wait
					.until(ExpectedConditions.visibilityOfElementLocated(STATUS_VALIDATION_MESSAGE_XPATH));
			...
			Assert.assertTrue(actualValidationText.contains(expectedPartialValidationMessage),
					"Validation message text mismatch. Expected to contain '" + expectedPartialValidationMessage
							+ "' but got '" + actualValidationText + "'");

			System.out.println("Test passed: Validation message displayed as expected (Input rejected).");

		} catch (org.openqa.selenium.TimeoutException e) {
			// This block is executed if the validation message does NOT appear within the
			// timeout.
			// This signifies that "nothing happened" (no error message) and the post might
			// have been uploaded.
			// As per your requirement, this scenario should cause the test to FAIL.
			Assert.fail("Test Failed: Validation message for empty status update did not appear within "
	...
	public void testPostTextAndVisibility() {
		...
		try {
			...
			WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(WAIT_TIMEOUT_SECONDS));
			WebElement postedTextElement = wait
					.until(ExpectedConditions.visibilityOfElementLocated(dynamicPostedTextXpath));
			...
			System.out.println("Test Passed: The text \"" + postText
					+ "\" was successfully posted and is visible on the timeline.");

		} catch (org.openqa.selenium.TimeoutException e) {
			Assert.fail("Test Failed: Posted text \"" + postText + "\" did not appear on the timeline within "
					+ WAIT_TIMEOUT_SECONDS + " seconds. " + e.getMessage());
		} catch (Exception e) {
	...
	public void testPostTimeVisibilityAndContent() {
		...
		// 3. Verify the posted text is present (optional, but good practice to ensure
		// the post itself is there)
		...
		try {
			...
			Assert.fail(
					"Pre-check failed: Posted text did not appear on timeline. Cannot verify time. " + e.getMessage());
		}
		...
		try {
			...
			Assert.fail("Test Failed: Post time element did not appear or was not visible within "
					+ WAIT_TIMEOUT_SECONDS + " seconds. " + e.getMessage());
		} catch (Exception e) {
	...
	public void testCommentingOnFirstPost() {
		...
		// If no post exists, the test will likely fail when trying to find the comment
		// box.
		...
		try {
			WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(WAIT_TIMEOUT_SECONDS));
			WebElement commentToggleButton = wait
					.until(ExpectedConditions.elementToBeClickable(COMMENT_TOGGLE_BUTTON_XPATH));
			commentToggleButton.click();
			System.out.println("Comment toggle/reveal button clicked.");
		} catch (org.openqa.selenium.TimeoutException e) {
			Assert.fail("Failed to find or click the comment toggle/reveal button within " + WAIT_TIMEOUT_SECONDS
		...
		try {
			...
			Assert.fail("Failed to find or interact with comment input box for the first post within "
					+ WAIT_TIMEOUT_SECONDS + " seconds. " + e.getMessage());
		}
		...
		try {
			WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(WAIT_TIMEOUT_SECONDS));
			WebElement commentSendButton = wait
					.until(ExpectedConditions.elementToBeClickable(COMMENT_SEND_BUTTON_XPATH));
			commentSendButton.click();
			System.out.println("Comment send button clicked for the first post.");
		} catch (org.openqa.selenium.TimeoutException e) {
			Assert.fail("Failed to find or click comment send button for the first post within " + WAIT_TIMEOUT_SECONDS
					+ " seconds. " + e.getMessage());
		}
		...
		try {
			...
			// This XPath assumes the comment text is directly within a <p> tag as per your
			// provided XPath structure for comments
			By dynamicCommentTextDisplayedXpath = By.xpath("//p[normalize-space()='" + commentText + "']");

			WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(WAIT_TIMEOUT_SECONDS));
			WebElement postedCommentElement = wait
					.until(ExpectedConditions.visibilityOfElementLocated(dynamicCommentTextDisplayedXpath));

			Assert.assertTrue(postedCommentElement.isDisplayed(),
					"Posted comment element is not displayed on the first post's timeline.");
			...
			System.out.println("Test Passed: The comment \"" + commentText
					+ "\" was successfully posted and is visible on the first post.");

		} catch (org.openqa.selenium.TimeoutException e) {
			Assert.fail("Test Failed: Posted comment \"" + commentText
					+ "\" did not appear on the first post's timeline within " + WAIT_TIMEOUT_SECONDS + " seconds. "
					+ e.getMessage());
		} catch (Exception e) {
			Assert.fail(
					"An unexpected error occurred during comment visibility test on the first post: " + e.getMessage());
		}
	}
