...
public class ButtonInteractionTest {
	...
	private final int SHORT_WAIT_SECONDS = 5; // Shorter wait for immediate changes like URL (used with FluentWait
												// timeout)
	...
	private final By ADD_PLACE_BUTTON_XPATH = By
			.xpath("//*[@id=\"root\"]/div/div/div[2]/div[2]/div/div[2]/div/div[2]/div[1]/div[1]");
	private final By TAG_FRIENDS_BUTTON_XPATH = By
			.xpath("//*[@id=\"root\"]/div/div/div[2]/div[2]/div/div[2]/div/div[2]/div[1]/div[2]");
	...
	public void testAddPlaceButtonFunctionality() {
		...
		try {
			...
			Assert.fail("Failed to find or click 'Add Place' button within " + WAIT_TIMEOUT_SECONDS + " seconds. "
					+ e.getMessage());
		}

		// 2. Check if URL changed using FluentWait. If not, it means "nothing happened"
		...
		try {
			...
			// If this condition becomes true, it means the URL changed (something
			// happened).
			fluentWait.until(driver -> !Objects.equals(driver.getCurrentUrl(), initialUrl));

			// If we reach here, the URL changed, meaning something happened, so the test
			// passes.
			System.out.println("Test Passed: URL changed from '" + initialUrl + "' to '" + driver.getCurrentUrl()
					+ "' after clicking 'Add Place'.");

		} catch (org.openqa.selenium.TimeoutException e) {
			// If TimeoutException occurs, it means the URL remained the same within the
			// FluentWait timeout.
	...
	public void testTagFriendsButtonFunctionality() {
		...
		try {
			...
			Assert.fail("Failed to find or click 'Tag Friends' button within " + WAIT_TIMEOUT_SECONDS + " seconds. "
					+ e.getMessage());
		}
