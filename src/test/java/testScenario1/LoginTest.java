...
public class LoginTest {
	...
	@Test(description = "Verify multiple error messages for empty username || Test Case ID TC_003", groups = {
			"negative", "regression" })
	public void testEmptyUsernameShowsMultipleErrorMessages() {
		...
		// The login method in LoginPage handles clicking the button after setting
		// credentials
		...
		try {
			...
			List<WebElement> errorMessageElements = wait
					.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(ERROR_MESSAGE_XPATH));

			// Define the expected error messages based on the HTML structure and common
			// validation
			List<String> expectedErrorMessages = Arrays.asList("Username is required.",
					"Please fill in all required fields.");

			// Extract the actual text from the found elements
			List<String> actualErrorMessages = errorMessageElements.stream().map(WebElement::getText).toList();
			...
			Assert.fail("Negative test failed: Error messages did not appear within " + WAIT_TIMEOUT_SECONDS
					+ " seconds for empty username. " + e.getMessage());
		} catch (Exception e) {
			Assert.fail("An unexpected error occurred during empty username test: " + e.getMessage());
		}
	}

	@Test(description = "Verify error message for invalid login credentials || Test Case ID TC_009", groups = {
			"negative", "regression" })
	public void testInvalidLoginShowsErrorMessage() {
		...
		try {
			...
			WebElement errorMessageElement = wait
					.until(ExpectedConditions.visibilityOfElementLocated(ERROR_MESSAGE_XPATH));
			...
			// You'll need to know what the actual error message text is for invalid
			// credentials
			// For example: "Invalid credentials", "Username or password incorrect", etc.
			String expectedPartialErrorMessage = "Login failed. Please check your credentials.";
			Assert.assertTrue(actualErrorMessage.contains(expectedPartialErrorMessage),
					"Error message text mismatch. Expected to contain '" + expectedPartialErrorMessage + "' but got '"
							+ actualErrorMessage + "'");

			System.out.println("Negative test passed: Error message displayed correctly for invalid login.");

		} catch (org.openqa.selenium.TimeoutException e) {
			// This block will execute if the error message doesn't appear within the
			// timeout
			Assert.fail("Negative test failed: Error message did not appear within " + WAIT_TIMEOUT_SECONDS
	...
	@Test(description = "Verify multiple error messages for empty password || Test Case ID TC_005", groups = {
			"negative", "regression" })
	public void testEmptyPasswordShowsMultipleErrorMessages() {
		...
		try {
			...
			List<String> expectedErrorMessages = Arrays.asList("Password is required.",
					"Please fill in all required fields.");
			...
			Assert.fail("Negative test failed: Error messages did not appear within " + WAIT_TIMEOUT_SECONDS
					+ " seconds for empty password. " + e.getMessage());
		} catch (Exception e) {
			Assert.fail("An unexpected error occurred during empty password test: " + e.getMessage());
		}
	}

	@Test(description = "Verify successful user login with valid credentials || Test Case ID TC_010", groups = {
			"regression", "positive" })
	public void testSuccessfulLogin() {
		...
		// We'll wait for a specific element on the home page (e.g., profileElement) to
		// become visible.
		// The XPath for profileElement is
		// "//*[@id="root"]/div/div/div[2]/div[1]/div/div/a[1]/div/img"
...
//        int WAIT_TIMEOUT_SECONDS = 15;
		try {
			...
			// Assert.assertTrue(driver.getCurrentUrl().contains("home"), "URL does not
			// contain 'home' after successful login.");

		} catch (org.openqa.selenium.TimeoutException e) {
			// This block will execute if the profile element doesn't appear within the
			// timeout
			Assert.fail("Login failed: Home page element (profile icon) did not become visible within "
